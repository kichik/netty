...
jfieldID packetLenFieldId = NULL;
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    if ((*vm)->GetEnv(vm, (void **) &env, JNI_VERSION_1_6) != JNI_OK) {
        ...
        packetLenFieldId = (*env)->GetFieldID(env, nativeDatagramPacketCls, "len", "I");
        if (packetLenFieldId == NULL) {
            throwRuntimeException(env, "Unable to obtain len field for NativeDatagramPacket");
            return JNI_ERR;
        }
...
JNIEXPORT jint JNICALL Java_io_netty_channel_epoll_Native_sendmmsg(JNIEnv * env, jclass clazz, jint fd, jobjectArray packets, jint offset, jint len) {
    struct msghdr hdrs[len];
    int i;
    for (i = 0; i < len; i++) {
         struct msghdr m = hdrs[i];
         struct iovec iov;
         struct sockaddr_storage addr;
         ...
         iov.iov_base = (void *) (*env)->GetLongField(env, packet, packetMemoryAddressFieldId);
         iov.iov_len = (size_t) (*env)->GetIntField(env, packet, packetLenFieldId);
